...
Repo-root `AGENTS.md` follows the same pure-prepend rule: a fork note sits above upstream's `# Overview` so any agent (Claude Code reads `AGENTS.md` when there is no `CLAUDE.md`, plus Codex, Cursor, etc.) is pointed here first. Upstream deleted `CLAUDE.md` in #7716 in favour of native `AGENTS.md` support — don't re-add a `CLAUDE.md`; it would shadow `AGENTS.md` for Claude Code and invite modify/delete conflicts.
...
- Enable `git config rerere.enabled true` locally — replays a resolved conflict automatically. Note `rerere`'s cache is **local-only**; CI aborts on conflict and fails the run; GitHub's workflow-failure email is the alert.
...
5. Rebase `fork` onto `$TAG` (`git rebase --onto "$TAG" "$(git merge-base fork upstream/main)" fork`), force-push (`--force-with-lease`, falling back to plain `--force`). On conflict: `git rebase --abort` and fail the job (the failure email is the alert; each hourly poll re-fails until resolved). Outputs the rebased commit as `sha`.
...
1. Checkout at `needs.prepare.outputs.sha`; `actions/download-artifact` with no `name:` filter, pulling every upstream job's artifacts into `artifacts/`, including `prepare`'s `cli-notes` artifact at `artifacts/cli-notes/cli-notes.md`.
2. Collect the DMG, `.app.tar.gz`, `.app.tar.gz.sig`, and any `anarlog-cli-*.tar.gz` into `dist/`. When `cli` ran, this means the CLI tarballs ship as GitHub release assets directly, in addition to going to the Homebrew tap in step 6. The `-name` filter only matches those four asset shapes, so `cli-notes.md` is never picked up as a release asset.
...
4. Compose human-readable release notes (`notes.md`), replacing the old one-line machine-generated body: a link to upstream's release notes (probes `packages/changelog/content/<version>.md` on `raw.githubusercontent.com`, since that file lags the tag by hours, falling back to the `desktop_v<version>` release page); DMG install instructions (delete `/Applications/Anarlog.app` first — same bundle identifier, the two builds cannot coexist; arm64 only; expect mic/system-audio re-prompts since the signing Team ID differs from upstream's); `brew install dave-atx/anarlog/anarlog-cli` for the CLI, followed by a generated "CLI changes since `<prev>`" subsection (the `cli-notes.md` bullets from `prepare`, or one line saying the CLI is unchanged when `cli-changed != 'true'` or notes are empty); a note that the Argmax STT models (ParakeetV2/V3, WhisperLargeV3) will not start in fork builds; and links to upstream and `FORK.md`.
...
| `fork/cliff.toml` | `git-cliff` config for the CLI-changes release notes — one bullet per commit message, upstream `(#1234)` refs linked to the upstream PR. Scope (`--include-path`) is passed on the command line by `prepare`'s CLI-notes step, see §8 |
